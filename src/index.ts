...
import type { CaptureType, TodoFilter } from "./lib/types.js";
...
function parseCaptureAddArgs(args: string[]): {
  type: CaptureType;
  text: string;
  tag?: string;
} {
...
function parseCaptureListArgs(args: string[]): { limit: number; type?: CaptureType } {
...
function parseOpenArgs(args: string[]): { query: string; printPath: boolean } {
  const textParts: string[] = [];
  let printPath = false;
...
function parseTodoFilter(rawFilter: string | undefined): TodoFilter {
  if (!rawFilter) {
    return "all";
  }
